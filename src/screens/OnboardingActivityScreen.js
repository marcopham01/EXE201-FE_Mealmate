...
export default function OnboardingActivityScreen({ route, navigation }) {
  const { goal } = route.params || {};
  const [activity, setActivity] = React.useState(null);

  const next = () => {
    navigation.navigate('OnboardingHeight', { goal, activity });
  };

  const Option = ({ label, value }) => (
    <TouchableOpacity
      onPress={() => setActivity(value)}
      style={[styles.option, activity === value && styles.optionActive]}
    >
      <Text style={[styles.optionText, activity === value && styles.optionTextActive]}>{label}</Text>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <View style={styles.progressBarBg}><View style={[styles.progressBarFill, { width: '28%' }]} /></View>
      </View>
      ...
      <Option label="Ít vận động" value="low" />
      <Option label="Vận động vừa phải" value="medium" />
      <Option label="Vận động nhiều" value="high" />

      <View style={{ flex: 1 }} />
      <TouchableOpacity disabled={!activity} onPress={next} style={[styles.nextBtn, !activity && { opacity: 0.6 }]}>
